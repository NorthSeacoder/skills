...
- 若当前工作明显是长期项目、仓库级实现或跨多轮持续维护，但尚无项目 `space` 约定，应主动建议用户创建项目级 `space`，而不是长期混写到 `Default`。
- 建议创建 `space` 的信号包括：
  - 当前目录是独立 git 仓库。
  - 任务会持续多轮迭代，且会积累决策、流程、调试经验。
  - 记忆需要和其他项目隔离检索，避免串扰。
  - 用户已经开始保存项目级实现知识，而不仅是一次性笔记。
- 给出建议时，默认建议使用项目名或仓库名作为 `space` 名；若已有稳定产品/系统名，优先用该名称。
- 若用户同意创建 `space`，后续搜索、去重、写入、更新、迁移都应切换到该 `space`，并把先前误写到 `Default` 的同类项目记忆迁入。
- 只有在没有项目约定、也没有现成 `space` 可用时，才使用 `Default`。
- 项目级知识应优先按 `space` 隔离；`labels` 仍用于模块、类型、版本等二级分类，不能代替 `space`。
...
- 命名应优先选服务端稳定、已验证可落库的名字；若带连字符的名字存在创建不稳定或不可见问题，优先使用无连字符形式。当前已验证应使用 `selfmedia`，不要再新建 `self-media`。
- 当用户要求“整理 Default”时，应先按高价值大类建立迁移批次，再执行 dry-run，确认后批量 move，而不是逐条零散搬运。
